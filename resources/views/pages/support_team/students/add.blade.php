            ...
            <fieldset>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <select onchange="getSubCounties(this.value)" required data-placeholder="Choose.." class="select-search form-control" name="state_id" id="state_id">
                            <option value=""></option>
                            @foreach($counties as $county)
                                <option {{ old('state_id') == $county->id ? 'selected' : '' }} value="{{ $county->id }}">{{ $county->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="col-md-3">
    <label for="lga_id">Sub-County: <span class="text-danger">*</span></label>
    <select required class="select-search form-control" name="lga_id" id="lga_id">
        <option value="">Select Sub-County</option>
        @foreach($sub_counties as $sub)
            <option value="{{ $sub->id }}" 
                @if(old('lga_id') == $sub->id) selected @endif
                data-county="{{ $sub->state_id }}">
                {{ $sub->name }}
            </option>
        @endforeach
    </select>
...
<script>
    function getSubCounties(countyId) {
        const subCountySelect = document.getElementById('lga_id');
        subCountySelect.innerHTML = '<option value="">Loading...</option>';
        subCountySelect.disabled = true;

        fetch(`/api/sub-counties/${countyId}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                subCountySelect.innerHTML = '<option value=""></option>';
        ...
        Array.from(subCountySelect.options).forEach(option => {
            option.style.display = 'block';
        });
        
        // Hide options that don't belong to selected county
        if (countyId) {
            Array.from(subCountySelect.options).forEach(option => {
                if (option.value && option.dataset.county !== countyId) {
